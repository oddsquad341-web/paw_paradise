...
import { ArrowRight, Star, Heart, Users, Leaf, Clock, Wind, Waves, Shield, X, ChevronLeft, ChevronRight, CheckCircle2, Play } from 'lucide-react';
...
function Lightbox({ images, index, onClose, onPrev, onNext }: {
  ...
  return (
    <div className="lightbox-overlay" onClick={onClose}>
      <button onClick={onClose} className="absolute top-5 right-5 text-white/70 hover:text-white z-10"><X size={28} /></button>
      <button onClick={(e) => { e.stopPropagation(); onPrev(); }}
        className="absolute left-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white z-10 p-2 rounded-full"
        style={{ background: 'rgba(255,255,255,0.1)' }}><ChevronLeft size={28} /></button>
      ...
      <button onClick={(e) => { e.stopPropagation(); onNext(); }}
        ...
        style={{ background: 'rgba(255,255,255,0.1)' }}><ChevronRight size={28} /></button>
...
const SERVICES = [
  { icon: Clock, title: '24/7 Dog Boarding', desc: 'Comfortable stays with personalized care. Round-the-clock supervision.', color: '#3f51a3', bg: '#eef0f8', href: '/services#boarding', image: '/assets/photos/dog-4.jpg' },
  { icon: Users, title: 'Dog Daycare', desc: 'Supervised social play. Structured activity and rest cycles daily.', color: '#58bd7a', bg: '#edf8f2', href: '/services#daycare', image: '/assets/photos/dog-6.jpg' },
  { icon: Wind, title: 'Grooming & Hygiene', desc: 'Professional bathing, brushing, nail trimming. Stress-free always.', color: '#3f51a3', bg: '#eef0f8', href: '/services#grooming', image: '/assets/photos/care-1.jpg' },
  { icon: Leaf, title: 'Diet & Nutrition', desc: 'Three meals a day, tailored to your dog\'s exact dietary needs.', color: '#58bd7a', bg: '#edf8f2', href: '/services#nutrition', image: '/assets/photos/dog-2.jpg' },
];
...
// Real gallery images from our photo shoot
const GALLERY_PREVIEW = [
  '/assets/photos/hero-1.jpg',
  '/assets/photos/dog-1.jpg',
  '/assets/photos/team-2.jpg',
  '/assets/photos/dog-9.jpg',
  '/assets/photos/facility-1.jpg',
  '/assets/photos/dog-7.jpg',
];
...
export default function Home() {
  ...
  const heroImages = ['/assets/photos/hero-wide-1.jpg', '/assets/photos/hero-wide-2.jpg', '/assets/photos/hero-wide-6.jpg'];
    ...
    <Layout>
      {lightbox !== null && (
        <Lightbox
          ...
          onPrev={() => setLightbox((lightbox - 1 + GALLERY_PREVIEW.length) % GALLERY_PREVIEW.length)}
          onNext={() => setLightbox((lightbox + 1) % GALLERY_PREVIEW.length)}
        />
      )}
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '95vh', background: '#1a1f3c' }}>
        ...
        {heroImages.map((img, i) => (
          <div key={img} className="absolute inset-0 bg-cover bg-center transition-opacity duration-1000"
            style={{ backgroundImage: `url(${img})`, opacity: i === heroImg ? 1 : 0 }}>
            <div className="absolute inset-0" style={{ background: 'linear-gradient(120deg, rgba(26,31,60,0.93) 0%, rgba(26,31,60,0.6) 55%, rgba(63,81,163,0.2) 100%)' }} />
          </div>
        ))}
        ...
        <div className="relative z-10 flex flex-col justify-center min-h-[95vh] max-w-7xl mx-auto px-6 py-20">
          <div className="max-w-2xl">
            <div className="badge-live animate-fade-up mb-5">Bhondsi, Gurgaon · Open Now</div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 animate-fade-up delay-300 mb-14">
              <a href="https://wa.me/919873218040?text=Hi%20Paw%20Paradise%2C%20I%27d%20like%20to%20book%20a%20service!" target="_blank" rel="noopener noreferrer" className="btn-green">
                Book via WhatsApp <ArrowRight size={16} />
              </a>
              <Link href="/gallery" className="btn-outline-white">See Our Gallery</Link>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 animate-fade-up delay-400">
              {STATS.map((s) => (
                <div key={s.label} className="rounded-2xl px-4 py-3" style={{ background: 'rgba(255,255,255,0.08)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.1)' }}>
                  <p className="text-2xl text-white" style={{ fontWeight: 800 }}><CountUp to={s.number} suffix={s.suffix} /></p>
        ...
        <div className="absolute bottom-16 left-1/2 -translate-x-1/2 flex gap-2 z-10">
          {heroImages.map((_, i) => (
            <button key={i} onClick={() => setHeroImg(i)}
              ...
              style={{ background: i === heroImg ? 'white' : 'rgba(255,255,255,0.35)', transform: i === heroImg ? 'scale(1.3)' : 'scale(1)' }} />
          ))}
        </div>
          ...
          <div className="reveal-right">
            <div className="grid grid-cols-2 gap-4">
              <div className="img-zoom rounded-2xl overflow-hidden shadow-xl" style={{ aspectRatio: '3/4' }}>
                <img src="/assets/photos/team-2.jpg" alt="Paw Paradise founder with dogs" className="w-full h-full object-cover" />
              </div>
              <div className="flex flex-col gap-4">
                <div className="img-zoom rounded-2xl overflow-hidden shadow-xl aspect-square">
                  <img src="/assets/photos/dog-9.jpg" alt="Dogs at Paw Paradise" className="w-full h-full object-cover" />
                </div>
      ...
      <section className="section max-w-7xl mx-auto px-6">
        <div className="text-center mb-14 reveal">
          ...
          <p className="body-lg mt-3 max-w-xl mx-auto" style={{ color: '#666' }}>Getting started is easy. Here's what booking with Paw Paradise looks like.</p>
        </div>
          ...
          {HOW_IT_WORKS.map((h, i) => (
            <div key={i} className="reveal bg-white rounded-2xl p-7 card-hover shadow-sm relative overflow-hidden group">
              <div className="absolute -right-2 -top-4 text-8xl font-800 select-none pointer-events-none transition-all duration-300 group-hover:scale-110"
                style={{ color: h.color, opacity: 0.06, fontWeight: 800, lineHeight: 1 }}>{h.step}</div>
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center mb-4 text-sm font-800"
                style={{ background: h.color, color: 'white', fontWeight: 800 }}>{h.step}</div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="reveal-right">
              <div className="rounded-3xl overflow-hidden shadow-2xl img-zoom relative">
                <img src="/assets/photos/hero-wide-5.jpg" alt="Paw Paradise facility" className="w-full object-cover" style={{ aspectRatio: '4/5' }} />
                <div className="absolute bottom-6 left-6 right-6 rounded-2xl p-4" style={{ background: 'rgba(26,31,60,0.85)', backdropFilter: 'blur(12px)' }}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="ml-auto"><div className="badge-live text-xs">Open</div></div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="section" style={{ background: '#1a1f3c' }}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-end justify-between mb-10 reveal">
            <div>
              <div className="eyebrow" style={{ color: '#58bd7a' }}>Our facility & dogs</div>
              <h2 className="display-lg text-white">Space to play. Space to rest.</h2>
            </div>
            <Link href="/gallery" className="hidden sm:flex btn-outline-white text-sm !py-2">Full gallery <ArrowRight size={14} /></Link>
          </div>
